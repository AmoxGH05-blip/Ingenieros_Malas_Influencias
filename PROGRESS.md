...
- [x] **SCRUM-32** — Tablas y relaciones del cuestionario (15 preguntas, escala 0/2.5/5/7.5/10) y respuestas (Responsable: Amoxhua Ochoa)
  - **Qué se hizo:** las tablas y relaciones ya existían desde SCRUM-24 (modelo adaptable `InstrumentoEvaluacion → SeccionInstrumento → Pregunta → OpcionRespuesta`) — lo que faltaba era sembrar el cuestionario real. Se creó un **instrumento borrador estándar** de 15 preguntas en 4 secciones (Dominio y organización de la materia, Metodología y evaluación, Puntualidad y responsabilidad, Trato y comunicación), cada una con las 5 opciones de la escala oficial (0 / 2.5 / 5 / 7.5 / 10). Además se agregaron las **entidades TypeORM** que faltaban para todo el dominio del cuestionario y respuestas (`TipoPregunta`, `InstrumentoEvaluacion`, `SeccionInstrumento`, `Pregunta`, `OpcionRespuesta`, `PeriodoEvaluacion`, `Evaluacion`, `RespuestaEvaluacion`) — listas para que Juan Antonio construya las APIs de SCRUM-33/34 encima, sin tener que mapearlas de cero.
  - **Cómo se hizo:** migración `database/migrations/003_cuestionario_evaluacion_docente.sql`, ejecutada y verificada contra `Central-Data` (15 preguntas, 75 opciones = 15×5, confirmado por conteo). Es un **borrador editable** — no el cuestionario oficial de la institución (el equipo no lo tenía a la mano); reemplazar el texto de las preguntas es solo actualizar filas, no tocar el esquema. Se creó también una cuenta `coordinacion@dygsis.local` (rol Coordinador) para ser el "creador" del instrumento — dato requerido por la FK, no un login funcional todavía.
  - **⚠️ Hallazgo (no es parte de este ticket, queda anotado para después):** la cuenta Coordinador creada **no puede iniciar sesión** con el login actual (SCRUM-26), porque `AuthService.login` solo resuelve el número de cuenta contra `Estudiante.Matricula` o `Docente.NumeroEmpleado` — Coordinador/Administrador no tienen ese concepto todavía. Falta definir cómo inician sesión esos dos roles (¿correo institucional en vez de número de cuenta?) antes de que SCRUM-42/43 (dashboard y panel de coordinación/administración) se puedan probar de verdad.
  - Archivos: `database/migrations/003_cuestionario_evaluacion_docente.sql`, `backend/src/entities/{TipoPregunta,InstrumentoEvaluacion,SeccionInstrumento,Pregunta,OpcionRespuesta,PeriodoEvaluacion,Evaluacion,RespuestaEvaluacion}.ts`.
